...
declare variable $local:places-records := (
   collection("C:\Users\anoni\Documents\GitHub\srophe\draft-data\data\places\tei\"), 
   collection("C:\Users\anoni\Documents\GitHub\srophe\draft-data\data\TSVTransformsXSLT\places\tei\")
 );
...
as node()+
{
  ...
  let $docId := substring-after($docId, "draft-data") (: hacked for draft-data; update this whenever you run on a different repo :)
  return
  <record>
    <uri>{$uri}</uri>
...
let $deprecatedUriList := local:create-uri-list($local:deprecated-records, "deprecated", $local:current-branch)
